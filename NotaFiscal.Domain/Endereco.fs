...
let createRua rua =
    let mapRuaErrors strError =
        ...
        | StringError.MustNotBeLongerThan 120 -> EnderecoError.RuaMustNotBeMoreThan120Chars
        | x -> x.GetType().Name |> failwithf "%s não esperado para o campo rua"
...
let createNumero numero =
    let mapNumeroErrors strError =
        ...
        | StringError.MustNotBeLongerThan 60 -> EnderecoError.NumeroMustNotBeMoreThan60Chars
        | x -> x.GetType().Name |> failwithf "%s não esperado para o campo numero"
...
let createComplemento complemento =
    let mapComplementoErrors strError =
        match strError with
        | StringError.MustNotBeLongerThan 60 -> EnderecoError.ComplementoMustNotBeMoreThan60Chars
        | x -> x.GetType().Name |> failwithf "%s não esperado para o campo complemento"
    
    StrMax60.createOptional complemento |> mapFailuresR mapComplementoErrors

let createBairro bairro =
    let mapBairroErrors strError =
        match strError with
        | StringError.Missing -> EnderecoError.BairroIsRequired
        | StringError.MustNotBeLongerThan 60 -> EnderecoError.BairroMustNotBeMoreThan60Chars
        | x -> x.GetType().Name |> failwithf "%s não esperado para o campo bairro"
...
let createCodigoMunicipio codigoMunicipio =
    let mapCodigoMunicipioErrors strErrors =
        ...
        | StringError.MustNotBeLongerThan 7 -> EnderecoError.CodigoMunicipioMustNotBeMoreThan7Chars
        | x -> x.GetType().Name |> failwithf "%s não esperado para o campo codigoMunicipio"
    
    StrMax7.create codigoMunicipio |> mapFailuresR mapCodigoMunicipioErrors
...
let createUF uf =
    let mapUfErrors strUfErrors =
        ...
        | x -> x.GetType().Name |> failwithf "%s não esperado para o campo uf"
...
let createCep cep =
    let mapCepErrors strCepErrors =
        ...
        | x -> x.GetType().Name |> failwithf "%s não esperado para o campo cep"
...
let createEndereco
    ...
    (cep: string): OperationResult<Endereco,EnderecoError> =
